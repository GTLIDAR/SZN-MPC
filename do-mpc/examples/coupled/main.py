...
x0 = np.array([[0], [0.001],[0], [0], [3],[0], [3]]) # x0 = np.array([[0], [0.001],[0], [0], [12]]) #204
...
def feedback_callback(msg):
    ...
    u0 = mpc.make_step(x0) # u0 = foot location
    # u0 = mpc.make_step([0,0,0.1,0.1])
    # print("NATHNAS U0",u0)

    y_next = simulator.make_step(u0) # send this to Digit using a python socket
    # print("PRANAVS YNEXT",y_next)
    # y_next[0] = msg.x_com[0]
    # y_next[1] = msg.xd_com[0]
    # y_next[2] = msg.dtheta[0]
    # y_next[3] = msg.x[0]
    # y_next[4] = msg.y[0]
    xth = estimator.make_step(y_next)
    # print(mpc.data.success[-1].item())
    ...
    mpc_data = PredictionState()
    # print(mpc.data.prediction(('_x', 'x',0))[0][1:])
    # print(mpc.data.prediction(('_x', 'x',1))[0][1:])
    # print(mpc.data.prediction(('_x', 'x',2))[0][1:])
    # print(mpc.data.prediction(('_x', 'x',3))[0][1:])

    mpc_data.x_com = mpc.data.prediction(('_x', 'x_com',0))[0][1:] #y_next[0]
    mpc_data.xd_com = mpc.data.prediction(('_x', 'xd_com',0))[0][1:]
    mpc_data.dtheta = mpc.data.prediction(('_x', 'd_theta',0))[0][1:]
    mpc_data.x = mpc.data.prediction(('_x', 'x_g',0))[0][1:]
    mpc_data.y = mpc.data.prediction(('_x', 'y_g',0))[0][1:]
    print(msg.x[0])
    print(mpc.data.prediction(('_x', 'x_g',0))[0][:])
    
    # mpc_data.x = mpc.data.prediction(('_x', 'x',0))[0][1:] #y_next[0]
    # mpc_data.xd = mpc.data.prediction(('_x', 'x',1))[0][1:]
    # mpc_data.y = mpc.data.prediction(('_x', 'x',2))[0][1:]
    # mpc_data.yd = mpc.data.prediction(('_x', 'x',3))[0][1:]
...
feedback_subscriber = rospy.Subscriber("/mpc_feedback", PredictionState, feedback_callback)

rospy.spin()


# send a last piece of data, setting the velocities to zero, so that Digit won't keep walking
# aimlessly in whatever direction he happened to be directed towards
# final_data = PredictionState()
# final_data.x = last_x
# final_data.y = last_y
# final_data.xd = 0
# final_data.yd = 0
# ros_publisher.publish(final_data)
...
# Store results:
if store_results:
    do_mpc.data.save_results([mpc, simulator], 'coupled_sgan_GP')
