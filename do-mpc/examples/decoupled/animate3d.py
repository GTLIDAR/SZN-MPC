...
def plot_polytope(center, generator_vectors, x, y, col,alpha, area, xlim=(0, 16), ylim=(0, 8)):
    ...
    for simplex in hull.simplices:
        ...
        z = vertices[simplex, 0]
        # ax.plot_trisurf(x, y, z, color='b', alpha=0.5, edgecolor='k')

    # xx, yy = np.meshgrid(vertices[hull.vertices,0], vertices[hull.vertices,1])
    # zz = 0*xx
...
# results_digitstl = load_results('./results/GP_sgan_full_decoupled_pedoptimal4.pkl')
results_digitstl = load_results('./results/004_iros_30.pkl')

# results_digitstl = load_results('./results/003_decoupled_wo_social_30.pkl')
ped_number = 30
...
for i in range(1,100,1):
    ...
    GP_G = results_digitstl['mpc'].prediction(('_tvp','GP_G')).reshape(1,5,2,2)
    # print(GP_G.shape)
    ...
    # print(GP_G[1:,:,:].shape)
    for n in range(horz):
        xx, yy, zz = plot_polytope(center_out_pred[:,n,0],GP_pred[n,:,:],0,0,'c',0.5,0)
        ax1.plot_trisurf(xx, yy, yy*0, color='b', alpha=0.5, edgecolor='k')

        # ax1.plot_trisurf(xx,yy,xx*0,color='c', alpha=0.5, ec='k')

    # for n in range(horz):
        # plot_polytope(center_pred[n,:],G_pred[n,:,:],x_pred[0,n+1],y_pred[0,n+1],'b')
        # plot_polytope(center_out_pred[:,n,0],G_pred[n,:,:],0,0,'c',0.2,0)
    ax1.plot(x_pred[0,:,0],y_pred[0,:,0],zs=0, color ='b')

    #
    ax1.scatter(10, 10,zs=0, marker="x", color = 'gold', s = 180)
    ...
    for ped in range(ped_number):
        ...
        ped_G1 = results_digitstl['mpc'].prediction(('_tvp',f'ped_G{ped}1'), t_ind=i).reshape(1,5,2)
        ped_G2 = results_digitstl['mpc'].prediction(('_tvp',f'ped_G{ped}2'), t_ind=i).reshape(1,5,2)
        ped_G3 = results_digitstl['mpc'].prediction(('_tvp',f'ped_G{ped}3'), t_ind=i).reshape(1,5,2)
        ped_G4 = results_digitstl['mpc'].prediction(('_tvp',f'ped_G{ped}4'), t_ind=i).reshape(1,5,2)
        ...
        if (obs_loc_pred[0,0,0] == 0.0 and obs_loc_pred[0,0,1] == 0.0):
            ...
            for n in range(horz):
                xx, yy = plot_polytope(obs_loc_pred[0,n,:],ped_G[:,n,:],0,0,'g',0.5,1)
                ax1.plot_surface(xx,yy,xx*0,color='g', alpha=0.5, ec='k')
